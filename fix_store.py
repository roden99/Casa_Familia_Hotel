...
const transformedColumns = computed(() =>
    props.columns
        ...
        .map(col => {
            if (col.accessorKey === 'pos_qty') {
                return {
                    ...col,
                    cell: ({ row }) => row.original.is_inventory ? (row.original.pos_qty ?? 0) : '-',
                };
            }
            return col;
        })
...
with open(
    r'C:\Projects\Working\xWorks_POS\resources\js\pages\StoreInventory\StoreInventoryIndex.vue',
    'w', newline='\n'
) as f:
    f.write(content)
