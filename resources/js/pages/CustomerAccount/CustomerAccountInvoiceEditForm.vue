...
const form = useForm({
    reference_no: props.invoice?.invoice_no && props.invoice.invoice_no !== '—' ? props.invoice.invoice_no : '',
    invoice_date: null,
    amount: props.invoice?.raw_amount ?? 0,
    terms: props.invoice?.terms ?? null,
...
onMounted(() => {
    if (props.invoice?.raw_date) {
        form.invoice_date = reverseDate(props.invoice.raw_date.slice(0, 10));
    }
...
<template>
    <FormCard :loading="isProcessing" size="md">
        <form @submit.prevent class="space-y-4 mt-4">
            <BaseField legend="Edit Invoice" description="Update this invoice entry">
                <template #fields>
                            ...
                            <Field class="col-span-12">
                                <div
                                    ...
                                    <div class="min-w-0 text-right">
                                        <span
                                            class="text-xs uppercase tracking-wide text-muted-foreground font-medium">Invoice</span>
                                        <p class="text-sm font-semibold font-mono truncate">{{ invoice?.reference ?? '—'
                                            }}</p>
                                    </div>
                                </div>
                            </Field>
