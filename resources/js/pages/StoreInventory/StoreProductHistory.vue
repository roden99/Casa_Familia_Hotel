...
onMounted(async () => {
    try {
        const res = await axios.get(`/store-inventory/${props.product.id}/history`, {
            headers: { Accept: 'application/json' },
        });
        productInfo.value = res.data.product;
        history.value = res.data.history;
    } catch (error) {
        toast.error('Failed to load store history.');
    } finally {
    ...
    <FormCard size="4xl" :loading="isLoading">
        <div class="space-y-5 mt-4">
            ...
            <div v-if="productInfo" class="rounded-lg border bg-muted/40 px-5 py-4">
                ...
                <div class="flex flex-wrap gap-6 text-sm text-muted-foreground">
                    <div class="flex flex-col gap-0.5">
                        <span class="text-xs uppercase tracking-wide font-medium">Current POS Qty</span>
                        <span class="text-foreground font-semibold text-lg">{{ productInfo.pos_qty }}</span>
                    </div>
                    <div class="flex flex-col gap-0.5">
                        <span class="text-xs uppercase tracking-wide font-medium">POS Unit</span>
                        <span class="text-foreground font-semibold">{{ productInfo.pos_unit }}</span>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="history.length === 0"
                ...
                No store history available for this product.
            </div>
                ...
                <Table>
                    <TableHeader>
                        <TableRow class="bg-muted/60 hover:bg-muted/60">
                            <TableHead class="w-20 font-semibold">Type</TableHead>
                            <TableHead class="font-semibold">Reference</TableHead>
                            <TableHead class="text-center w-20 font-semibold">Qty</TableHead>
                            <TableHead class="text-center w-24 font-semibold">Multiplier</TableHead>
                            <TableHead class="text-right w-24 font-semibold text-green-700 dark:text-green-400">
                                IN
                            </TableHead>
                            <TableHead class="text-right w-24 font-semibold text-red-600 dark:text-red-400">
                                OUT (Sold)
                            </TableHead>
                    ...
                    <TableBody>
                        <TableRow v-for="(entry, index) in history" :key="index" :class="[
                            entry.type === 'INITIAL' ? 'bg-muted/30 hover:bg-muted/40' : '',
                            ...
                            <TableCell class="text-base text-muted-foreground">{{ entry.reference }}</TableCell>
                            <TableCell class="text-center font-mono text-sm">
                                {{ entry.type !== 'INITIAL' ? entry.qty_raw : '—' }}
                            </TableCell>
                            <TableCell class="text-center font-mono text-sm">
                                {{ (entry.type === 'IN') ? ('× ' + entry.multiplier) : '—' }}
                            </TableCell>
                            <TableCell
                            ...
                            <TableCell
                                class="text-right font-mono text-base font-medium text-red-600 dark:text-red-400">
